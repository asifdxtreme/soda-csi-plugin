...
func NewIdentityServer(d *csicommon.CSIDriver, c kubernetes.Interface) *identityServer {
...
func (lvm *lvm) Run(driverName, nodeID, endpoint string, vgName string) {
	glog.Infof("Driver name ............: %v ", driverName)
	...
	// Create GRPC servers
	lvm.ids = NewIdentityServer(lvm.driver, lvm.client)
